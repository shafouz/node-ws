...
describe("rooms work", () => {
  beforeAll(async () => {
    await page.goto("http://localhost:8000/?room=1");

    const form = await page.$("#message");
    await form.type("this is room 1");
  ...
  it("send message", async () => {
    ...
    expect(obj.user.length).toEqual(16);
    expect(obj.user_len).toEqual(1);
    expect(obj.message.includes("this is room 1")).toEqual(true);
    expect(obj.messages_len).toEqual(1);

    const incognito = await browser.createIncognitoBrowserContext();
    const page2 = await incognito.newPage();
    await page2.goto("http://localhost:8000/?room=1");
    const form2 = await page2.$("#message");
    await form2.type("this is room 2");
    const button2 = await page2.$("#sendMessage");
    await button2.click();
    ...
    await page2.goto("http://localhost:8000/?room=10");
    const users_page_room_0 = await page2.$$("#users > b");
    expect(users_page_room_0.length).toEqual(1);
  });
});
